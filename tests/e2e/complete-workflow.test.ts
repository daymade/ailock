...
describe("AILock E2E Complete Workflow Tests", () => {
  ...
  describe("Lock/Unlock Workflow", () => {
    ...
    it("should lock and unlock files correctly", () => {
      ...
      // Unlock file
      const unlockOutput = execSync(`${ailock} unlock .env`, {
        encoding: "utf8",
      });
    ...
    it("should support dry-run mode", () => {
      const output = execSync(`${ailock} lock .env --dry-run`, {
        encoding: "utf8",
      });
  ...
  describe("Status Command", () => {
    it("should show correct status", () => {
      ...
      // Check status
      const output = execSync(`${ailock} status --json --skip-analytics`, {
        encoding: "utf8",
      });
  ...
  describe("Cross-Command Integration", () => {
    it("should maintain consistency across commands", () => {
      ...
      const statusOutput = execSync(
        `${ailock} status --json --skip-analytics`,
        {
          encoding: "utf8",
        },
      );
      ...
      const hasEnvFile = status.lockedFiles.some((file: string) =>
        file.endsWith(".env"),
      );
      ...
      const statusAfter = execSync(`${ailock} status --json --skip-analytics`, {
        encoding: "utf8",
      });
      const statusObj = JSON.parse(statusAfter);
      const hasEnvFileAfter = statusObj.lockedFiles.some((file: string) =>
        file.endsWith(".env"),
      );
